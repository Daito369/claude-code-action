...
jobs:
  claude-response:
    ...
    steps:
      - uses: Daito369/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: Daito369/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`Daito369/claude-code-base-action`](https://github.com/Daito369/claude-code-base-action).
